		...
		struct drm_plane_state *state)
{
	...
	if (plane_enabled(state)) {
		...
		hflip = !!(state->rotation & BIT(DRM_MODE_REFLECT_X));
		vflip = !!(state->rotation & BIT(DRM_MODE_REFLECT_Y));
		...
		uint32_t src_w, uint32_t src_h)
{
	...
	hflip = !!(pstate->rotation & BIT(DRM_MODE_REFLECT_X));
	vflip = !!(pstate->rotation & BIT(DRM_MODE_REFLECT_Y));
